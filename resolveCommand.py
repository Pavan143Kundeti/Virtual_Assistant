...
def resolve_command(command):
    ...
    if isContain(command, ["calculate", "what is the value of", "value of", "perform", "evaluate", "find the value of", "find"]):
    ...
    elif "set a timer for" in command or 'start a timer for' in command or 'set timer for' in command:
        ...
        return sys.resolveCommand(command)
    ...
    if isContain(command, ["translate", " to ", "how to say", "how do you say", " in "]):
        ...
        open_price = stock_info.get('regularMarketOpen')
        high_price = stock_info.get('dayHigh')
        low_price = stock_info.get('dayLow')
        ...
        print('For more information, visit https://finance.yahoo.com/quote/AAPL?p=AAPL')
        return True
